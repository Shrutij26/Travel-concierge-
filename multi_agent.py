...
def build_multi_agent_graph(llm, tools_dict, checkpointer):
    ...
    local_guide_agent = create_react_agent(
        ...
        prompt="You are a Local Guide Agent. Use your tools to find top attractions and search the travel knowledge base."
    )
    
    logistics_agent = create_react_agent(
        llm, 
        tools=tools_dict.get("logistics_tools", []),
        prompt="You are a Logistics Agent. Your job is to fetch the current weather and generate map coordinates for requested places."
    )
    
    budget_agent = create_react_agent(
        llm, 
        tools=tools_dict.get("budget_tools", []),
        prompt="You are a Budget Agent. Your job is to search the web for cheap flights, hotel deals, and budget tips."
    )
